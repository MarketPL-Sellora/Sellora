...
public class ProductChooseE2ETest {
  ...
  void testProductChoosing() {
    ...
    // Чекаємо, поки посилання стане видимим (це означає, що воно відрендерилось)
    productLink.waitFor();

    // Клікаємо
    productLink.click();

    // 7. Перевірка, що ми на сторінці товару
    // (наприклад, перевіряємо, що URL змінився)
    page.waitForURL("**/product/2");
    assertTrue(page.url().contains("/product/2"));
  }
